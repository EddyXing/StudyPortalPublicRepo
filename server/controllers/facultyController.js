...
module.exports = {
    ...
    markAttendance: async (req, res, next) => {
        try {
            ...
            var filteredArr = allStudents.filter(function (item) {
                return selectedStudents.indexOf(item._id.toString()) === -1;
            });

            //Absent Bacche
            for (let i = 0; i < filteredArr.length; i++) {
                const pre = await Attendance.findOne({ student: filteredArr[i]._id, subject: sub._id })
                if (!pre) {
            ...
            for (var a = 0; a < selectedStudents.length; a++) {
                const pre = await Attendance.findOne({ student: selectedStudents[a], subject: sub._id })
                if (!pre) {
                    const attendance = new Attendance({
    ...
    uploadMarks: async (req, res, next) => {
        try {
            ...
            const { subjectCode, exam, totalMarks, marks, department,
                section } = req.body
    ...
    updateProfile: async (req, res, next) => {
        try {
            ...
            const { email, gender, facultyMobileNumber,
                aadharCard } = req.body
            if (!gender || !facultyMobileNumber || !aadharCard) {
